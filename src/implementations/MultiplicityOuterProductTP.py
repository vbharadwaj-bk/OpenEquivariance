...
class MultiplicityOuterProductTP(TensorProduct):
    def __init__(self, config : TPProblem, torch_op : bool = False):
        ...
        forward_smem_gemm_weights_scratch = max(RepData(config.irreps_out).mults) * forward_smem_gemm_max_n

        forward_smem_gemm_info = {
            'n' : forward_smem_gemm_max_n,
            'L1L2_scratch_elems' : forward_smem_gemm_L3_scratch,
            'weight_scratch_elems' : forward_smem_gemm_weights_scratch,
        }
        ...
        forward_smem_per_warp['gemm_weights'] = (forward_smem_gemm_weights_scratch * sizeof('float'))
        ...
        backward_smem_per_warp['gemm_weights'] = (backward_semem_gemm_weights_scratch * sizeof('float'))
