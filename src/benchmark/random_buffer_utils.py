...
def get_random_buffers_forward_conv(
        ...
        node_count: int,
        edge_count: int,
        prng_seed : int) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    assert isinstance(tpp, TPProblem)
    rng = np.random.default_rng(prng_seed)

    in1 = np.array(rng.uniform(size=(node_count, tpp.irreps_in1.dim)), dtype=tpp.irrep_dtype) 
    in2 = np.array(rng.uniform(size=(edge_count, tpp.irreps_in2.dim)), dtype=tpp.irrep_dtype)

    weights_size = tuple([tpp.weight_numel]) if tpp.shared_weights else tuple([edge_count, tpp.weight_numel]) 
    weights = np.array(rng.uniform(size=weights_size), dtype=tpp.weight_dtype)
...
def get_random_buffers_backward_conv(tpp : TPProblem, node_count: int, edge_count: int, prng_seed : int) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """
    Return properly sized numpy arrays needed to execute a tensor product in the backward direction
    Supports shared vs non-shared weights
    """
    ...
    in1 = np.array(rng.uniform(size=(node_count, tpp.irreps_in1.dim)), dtype=tpp.irrep_dtype) 
    in2 = np.array(rng.uniform(size=(edge_count, tpp.irreps_in2.dim)), dtype=tpp.irrep_dtype)
    out_grad = np.array(rng.uniform(size=(node_count, tpp.irreps_out.dim)), dtype=tpp.irrep_dtype)
