...
PYBIND11_MODULE(hip_wrapper, m) {
    ...
    py::class_<JITTPImpl<HIPJITKernel>, GenericTensorProductImpl>(m, "JITTPImpl")
    ...
    py::class_<JITConvImpl<HIPJITKernel>, ConvolutionImpl>(m, "JITConvImpl")
        .def(py::init<std::string, KernelLaunchConfig&, KernelLaunchConfig&>());
    ...
    py::class_<PyDeviceBuffer<HIP_Allocator>>(m, "DeviceBuffer")
        .def(py::init<uint64_t>())
        .def(py::init<py::buffer>())
        .def("copy_to_host", &PyDeviceBuffer<HIP_Allocator>::copy_to_host)
        .def("data_ptr", &PyDeviceBuffer<HIP_Allocator>::data_ptr);
